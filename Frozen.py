...
class main():
    ...
    def print_data(self, ll, p_get_time, sysj, data):
        try:
            send_data = re.findall('68.*?16', data)[0]
        except:
            send_data = data
        p_datas = '\n{xx}、剩余次数：{a} | 设置的时间：{b} | 预计剩余时间：{c}\n     |当前时间[{d}]\n     |发送:{e}'.format(xx=str(self.L-ll),
            a=str(ll), b=p_get_time, c=sysj, d=timen(), e=send_data)
        print_save(p_datas)
...
def if_lead(serss):
    global IF_LEAD
    ifLead = input('是否增加前导：1、是，2、否\n')
    if ifLead == '1' or ifLead == '是':
        print('增加前导码, 串口波特率为2400')
        serss.baudrate = 2400
        IF_LEAD = True
    else:
